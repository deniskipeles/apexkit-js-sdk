...
export class ApexKit {
    ...
    get admins() {
        return {
            ...
            createCollection: (name: string, schema: any) => this._request<Collection>('/collections', { method: 'POST', body: { name, schema } }),
            getCollection: (id: string | number) => this._request<Collection>(`/collections/${id}`),
            updateCollection: (id: string | number, payload: any) => this._request<Collection>(`/collections/${id}`, { method: 'PUT', body: payload }),
            patchCollection: (id: string | number, payload: any) => this._request<Collection>(`/collections/${id}`, { method: 'PATCH', body: payload }),
            deleteCollection: (id: string | number) => this._request(`/collections/${id}`, { method: 'DELETE' }),

            // Config
            listConfigs: () => this._request<any[]>('/admin/config'),
            setConfig: (key: string, value: string, encrypt: boolean) => this._request('/admin/config', { method: 'POST', body: { key, value, encrypt } }),
            deleteConfig: (key: string) => this._request(`/admin/config/${encodeURIComponent(key)}`, { method: 'DELETE' }),

            // Users
            registerUser: (email: string, password?: string, role?: string, metadata?: any) =>
                this._request<AuthResponse>('/auth/register', { method: 'POST', body: { email, password, role, metadata } }),

            updateUser: (id: string | number, email?: string, password?: string, role?: string, metadata?: any) =>
                this._request<User>(`/admin/users/${id}`, { method: 'PATCH', body: { email, password, role, metadata } }),

            listUsers: (options: QueryOptions = {}) => this._request<ListResult<User>>(`/admin/users`, { method: 'GET', params: options }),
            deleteUser: (id: string | number) => this._request(`/admin/users/${id}`, { method: 'DELETE' }),

            // Settings
            getSettings: () => this._request<any>('/admin/settings'),
            updateSettings: (settings: any) => this._request<any>('/admin/settings', { method: 'PUT', body: settings }),
            patchSettings: (settings: any) => this._request<any>('/admin/settings', { method: 'PATCH', body: settings }),

            // Storage Utils
            testS3StorageConnection: (config: any) => this._request('/admin/storage/test', { method: 'POST', body: config }),
            migrateStorage: (source: string, destination: string) => this._request<any>('/admin/storage/migrate', { method: 'POST', body: { source, destination } }),

            // Backups
            createBackup: () => this._request('/admin/backup', { method: 'POST' }),
            listBackups: () => this._request<Array<{ name: string, size: number, created: string }>>('/admin/backups', { method: 'GET' }),
            restoreFromFile: (filename: string) => this._request('/admin/restore-file', { method: 'POST', body: { filename } }),
            ...
            createApiKey: (name: string, role = 'admin', scope = 'root', bypass_cors = true) =>
                this._request<{ key: string, info: ApiKey }>('/admin/keys', { method: 'POST', body: { name, role, scope, bypass_cors } }),
            updateApiKey: (id: string | number, updates: any) => this._request(`/admin/keys/${id}`, { method: 'PUT', body: updates }),
            ...
            revectorizeCollection: (collectionId: string | number) =>
                this._request(`/admin/collections/${collectionId}/revectorize`, { method: 'POST', body: { force: false } }),
            ...
            // Tenant Admin Ops
            createTenant: (tenantId: string) => this._request('/admin/tenants', { method: 'POST', body: { tenant_id: tenantId } }),
            deleteTenant: (id: string) => this._request(`/admin/tenants/${id}`, { method: 'DELETE' }),
            updateTenant: (id: string, data: any) => this._request(`/admin/tenants/${id}`, { method: 'PATCH', body: data }),
    ...
    get ai() {
        return {
            ...
            createAction: (data: Partial<AiAction>) => this._request('/admin/ai/actions', { method: 'POST', body: data }),
            deleteAction: (id: string | number) => this._request(`/admin/ai/actions/${id}`, { method: 'DELETE' }),
            run: (slug: string, variables: Record<string, any>) =>
                this._request<{ result: string, metadata: any }>(`/ai/run/${slug}`, { method: 'POST', body: { variables } }),

            // Architect
            listSessions: () => this._request<AiSession[]>('/admin/ai/sessions'),
            createSession: (name: string, initialPrompt?: string, model?: string, cloneStrategy?: string, cloneRecordLimit?: number) =>
            ...
            deleteSession: (id: string) => this._request(`/admin/ai/sessions/${id}`, { method: 'DELETE' }),
            chat: (sessionId: string, prompt: string, model: string) =>
                this._request<AiSession>(`/admin/ai/sessions/${sessionId}/chat`, { method: 'POST', body: { prompt, model } }),
            ...
            editCode: (prompt: string, currentCode: string, contextType: string, model: string) =>
                this._request<{ code: string }>('/admin/ai/edit-code', { method: 'POST', body: { prompt, current_code: currentCode, context_type: contextType, model } }),
    ...
    get templates() {
        return {
            ...
            create: (data: Partial<Template>) => this._request<Template>('/admin/templates', { method: 'POST', body: data }),
            update: (id: string | number, data: Partial<Template>) => this._request(`/admin/templates/${id}`, { method: 'PUT', body: data }),
            delete: (id: string | number) => this._request(`/admin/templates/${id}`, { method: 'DELETE' }),
            export: async (): Promise<Blob> => {
    ...
    collection(collectionId: string | number) {
        return {
            list: (options: QueryOptions = {}) => this._request<ListResult<BaseRecord>>(`/collections/${collectionId}/records`, { method: 'GET', params: options }),

            searchRecordsWithSQL: (query: any) => this._request<BaseRecord[]>(`/collections/${collectionId}/query`, { method: 'POST', body: { query } }),
            searchRecordsWithOSE: (query: string) => this._request<BaseRecord[]>(`/collections/${collectionId}/search`, { method: 'GET', params: { q: query } }),
            searchRecordsInstantlyWithOSE: (query: string) => this._request<InstantResult[]>(`/collections/${collectionId}/instant-search`, { method: 'GET', params: { q: query } }),

            create: (data: any) => this._request<BaseRecord>(`/collections/${collectionId}/records`, { method: 'POST', body: { data } }),
            get: (recordId: string | number, options: { expand?: string } = {}) => this._request<BaseRecord>(`/collections/${collectionId}/records/${recordId}`, { method: 'GET', params: options }),
            update: (recordId: string | number, data: any) => this._request<BaseRecord>(`/collections/${collectionId}/records/${recordId}`, { method: 'PUT', body: { data } }),
            patch: (recordId: string | number, data: any) => this._request<BaseRecord>(`/collections/${collectionId}/records/${recordId}`, { method: 'PATCH', body: { data } }),
            delete: (recordId: string | number) => this._request(`/collections/${collectionId}/records/${recordId}`, { method: 'DELETE' }),

            addRelation: (originRecordId: string | number, targetCollectionId: string | number, targetRecordId: string | number, relationName: string) =>
                this._request(`/collections/${collectionId}/records/${originRecordId}/relations`, {
                    method: 'POST',
                    body: { target_collection_id: Number(targetCollectionId), target_record_id: Number(targetRecordId), relation_name: relationName }
                }),

            removeRelation: (originRecordId: string | number, targetCollectionId: string | number, targetRecordId: string | number, relationName: string) =>
                this._request(`/collections/${collectionId}/records/${originRecordId}/relations`, {
                    method: 'DELETE',
                    body: { target_collection_id: Number(targetCollectionId), target_record_id: Number(targetRecordId), relation_name: relationName }
                }),
            ...
            searchVector: (field: string, vector: number[], limit = 10) =>
                this._request<BaseRecord[]>(`/collections/${collectionId}/search-vector`, { method: 'POST', body: { field, vector, limit } }),

            searchTextVector: (queryText: string, limit = 10) =>
                this._request<BaseRecord[]>(`/collections/${collectionId}/search-text-vector`, { method: 'POST', body: { query_text: queryText, limit } }),

            searchImageVector: (imageData: string, limit = 10) =>
                this._request<BaseRecord[]>(`/collections/${collectionId}/search-image-vector`, { method: 'POST', body: { image_data: imageData, limit } }),

            getVector: (recordId: string | number) =>
                this._request<Array<{ field_name: string, vector: number[], model: string }>>(`/collections/${collectionId}/get-vector/${recordId}`, { method: 'GET' }),
        };
    }

    get files() {
        return {
            list: (page = 1, perPage = 20) => this._request<ListResult<StoredFile>>('/storage/files', { method: 'GET', params: { page, per_page: perPage } }),
    ...
    get sites() {
        return {
            ...
            delete: (path: string) => this._request('/admin/site/files', { method: 'DELETE', params: { path } })
        };
    }
}
